...
def sample_target(n: int, rng: np.random.Generator) -> np.ndarray:
    return MU_TARGET + SIGMA_TARGET * rng.standard_t(df=NU_TARGET, size=n)
...
def get_ground_truth_velocity(
    ...
    """Double-latent Monte Carlo estimate of u*(t, x) for all source cases."""
...
def hill_estimator(samples: np.ndarray, frac: float = 0.05, center: float = MU_TARGET) -> float:
...
def plot_velocity_timestep_grid_for_dof(
    ...
    for i, t in enumerate(t_values):
        ...
        # Reference lines for source/target means.
        ax.axvline(0.0, color="gray", lw=0.8, alpha=0.35, linestyle="-", zorder=1, label="p0 mean")
        ax.axvline(MU_TARGET, color="black", lw=0.8, alpha=0.45, linestyle="-", zorder=1, label="p1 mean")
...
def plot_histogram_comparison(
    ...
    pdf = student_t_pdf(x, MU_TARGET, SIGMA_TARGET, NU_TARGET)

    fig, ax = plt.subplots(1, 1, figsize=(9, 5))
    ax.plot(x, pdf, color="black", lw=0.5, label=r"Analytical $\mathcal{T}_{\nu=3}(\mu=4)$ PDF")
...
def compute_metrics(samples: np.ndarray, ref_samples: np.ndarray) -> dict[str, float]:
    return {
        "hill": hill_estimator(samples, frac=0.05, center=MU_TARGET),
...
def run_experiment(args: argparse.Namespace) -> None:
    ...
    run_tag = f"single_mu{_format_hparam(MU_TARGET)}_t{time_tag}"
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Heavy-tail flow matching sample-complexity experiment")
    ...
    p.add_argument("--time_sampling", type=str, choices=["uniform", "beta"], default="beta")
    p.add_argument("--beta_alpha", type=float, default=5.0)
    p.add_argument("--beta_beta", type=float, default=7.0)
    p.add_argument("--x_min", type=float, default=-10.0)
    p.add_argument("--x_max", type=float, default=15.0)
